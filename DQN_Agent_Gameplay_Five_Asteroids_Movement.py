...
class Game:
    ...
    def get_reward(self):
        ...
        reward = 0
        # Assign a positive reward for destroying an asteroid
        reward += self.score  # Assuming self.score is incremented elsewhere in the code when an asteroid is 
        ...
        min_safe_distance = 50  # Define what you consider a close encounter (in pixels)
...
class Player:
    def __init__(self, game):
        ...
        self.bullet_cooldown_ms = 1000  # 1000 ms divided by 4 bullets per second
...
class Bullet:
    def __init__(self, game, position, angle):
        ...
        self.lifetime = 60  # Frames until the bullet is automatically removed
...
class DQNAgent:
    def __init__(self, state_size, action_size, replay_buffer_size=100000, update_target_every=5):
...
model = load_model('C:/Users/ericl/OneDrive/Documents/ReinforcementLearning/DQN_Models/model_best_movement_incentive.h5')

# Step 2: Initialize the game environment
env = Game()  # Assuming 'Game' is your game class

# Optionally, set the game to a state that's ready for rendering
pygame.init()

def play_game_with_model(env, model):
    state = env.reset()
    ...
    while not done:
        ...
        # Step 3: Use the model to predict the next action
        ...
        for event in pygame.event.get():  # Allow manual closing of the window
            if event.type == pygame.QUIT:
                done = True

    print("Game over with total reward:", total_reward)

# Run the game
play_game_with_model(env, model)
